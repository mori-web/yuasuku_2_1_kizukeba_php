...
<body>
  <?php
    try {
        ...
        $post = sanitize($_POST);
        $pro_code = $post['code']; //前の画面からpostメソッドから受け取ったデータを変数に代入
        $pro_name = $post['name']; //前の画面からpostメソッドから受け取ったデータを変数に代入
        $pro_price = $post['price']; //前の画面からpostメソッドから受け取ったデータを変数に代入
        $pro_gazou_name_old = $post['gazou_name_old']; //前の画面からpostメソッドから受け取ったデータを変数に代入
        ...
        $sql = 'UPDATE mst_product SET name=?,price=?,gazou=? WHERE code=?'; //SQL文を変数に代入
