...
<body>
  ...
  try {
      ...
      $sql = 'SELECT code,name FROM mst_staff WHERE 1'; //SQL文で全てのレコードを取得して、変数に代入
      ...
      while(true) { //データーベースから取得したデータを表示
          ...
          print '<input type="radio" id="'. $rec['code'].'" name="staffcode" value="' . $rec['code'] . '">';
          print '<label for="'. $rec['code'] .  '">'. $rec['name'] .'</label>';
          print '<br/>';
      }
